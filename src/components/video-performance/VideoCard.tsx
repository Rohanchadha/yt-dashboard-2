...
const TIPS: Record<string, string> = {
  ...
  "CTR EFF": "Your video's CTR ÷ channel average CTR. >1× = above-average thumbnail performance.",
...
function getValue(video: VideoWithMetrics, col: string): string | number {
  switch (col) {
    ...
    case "CTR EFF":     return video.derived.ctrEfficiency ?? -1;
          ...
          {sorted.map((video, idx) => {
            ...
            return (
              <tr key={video.id} style={{ background: idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
                ...
                <td style={{ ...TD_STYLE, color: video.derived.ctrEfficiency != null ? "#06b6d4" : "var(--text-secondary)" }}>
                  {video.derived.ctrEfficiency != null ? `${video.derived.ctrEfficiency}×` : "—"}
                </td>
