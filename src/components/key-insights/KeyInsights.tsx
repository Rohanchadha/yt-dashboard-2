...
import { TrendingUp, Users, Eye, Clock, Share2, Trophy, Target, Zap } from "lucide-react";
...
export default function KeyInsights({ data }: { data?: Partial<DashboardData> }) {
  ...
  const insights = [
    { icon: Trophy, color: "#f59e0b", bg: "#f59e0b22", title: "Best Performing Video", value: bestVideo.titleShort, detail: `${bestVideo.views.toLocaleString()} views · ${bestVideo.derived.classification.replace("_", " ")}` },
    ...
    { icon: Clock, color: "#a855f7", bg: "#a855f722", title: "Longest Avg Watch Time", value: bestRetentionVideo.titleShort, detail: `${bestRetentionVideo.avgDurationSec}s avg · ${bestRetentionVideo.watchTimeHours}h total` },
    ...
    { icon: Eye, color: "#f97316", bg: "#f9731622", title: "Engagement Rate", value: `${summary.engagedPct}%`, detail: `${summary.engagedViews.toLocaleString()} of ${summary.totalViews.toLocaleString()} viewers engaged` },
    ...
    { icon: Share2, color: "#06b6d4", bg: "#06b6d422", title: "Most Subscribers Gained", value: highestSubsVideo.titleShort, detail: `${highestSubsVideo.subsGained} subscribers gained from this video` },
  ];
        ...
        {insights.map((insight) => {
          ...
          return (
            <div key={insight.title} className="card p-5 flex flex-col gap-3">
              ...
              <div className="text-lg font-bold leading-tight" style={{ color: "var(--text-primary)" }}>{insight.value}</div>
